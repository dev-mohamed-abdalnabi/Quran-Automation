...
if __name__ == "__main__":
    ...
    # لو فيديو النهارده اتنشر خلاص → اقفل
    if is_uploaded_today():
        print("✅ فيديو النهارده متسجّل إنه اتنشر بالفعل. مفيش شغل.")
        sys.exit(0)

    print("❌ مفيش فيديو متسجّل للنهارده. هنحاول نعمل ونرفع...")

    try:
        build_shorts_video()
        # لو وصلنا هنا من غير Exception → نعتبر الرفع نجح
        mark_uploaded_today()
        print("📝 تم تحديث السجل اليومي بنجاح.")
    except Exception as e:
        print("🔥 حصل خطأ أثناء التنفيذ أو الرفع:", e)
        # نطلع بخطأ عشان GitHub يعرف إن الرن فشل ويعيد المحاولة في الرن الجاية
        sys.exit(1)
